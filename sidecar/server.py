...
def logs_messages(char: str, partner: str, offset: int = 0, limit: int | None = None) -> dict:
    ...
    try:
        ...
        # written under "Daelan Envale" pre-rename still apply when
        # the user opens the merged "Asmira" conversation.
...
# ---- flist working sets / snapshots / backups (Tier 7) ---------------
...
def _set_meta_to_json(meta: character_archive.SetMeta) -> dict:
    return {
        ...
        "snapshotCount": meta.snapshot_count,
    }


def _snapshot_meta_to_json(meta: character_archive.SnapshotMeta) -> dict:
    return {
        "id": meta.id,
        "name": meta.name,
        "createdAt": meta.created_at,
...
def _backup_listing_to_json(row: character_archive.BackupListing) -> dict:
    return {
...
class SetCreateRequest(BaseModel):
    ...
    # seed is "live" | "empty" | {"fork": "<set_id>"}; validated below
    # because pydantic's union shape would shadow our nicer error.
    seed: Any
...
class BackupCreateRequest(BaseModel):
    source: Literal["set", "snapshot"]
    set_id: str = Field(min_length=1, max_length=32)
    snapshot_id: str | None = None


@app.get("/flist/character/{character_id}/sets")
async def flist_sets_list(character_id: str) -> dict:
    # Trigger v5 → v6 migration on first read so legacy archives surface
    # their single Main set without a separate user action.
    character_archive.migrate_v5_working_to_sets(character_id)
    sets = character_archive.list_sets(character_id)
    return {
        "sets": [_set_meta_to_json(s) for s in sets],
...
async def flist_sets_create(character_id: str, body: SetCreateRequest) -> dict:
    seed = body.seed
    if isinstance(seed, str):
        if seed not in ("live", "empty"):
            raise HTTPException(
                status_code=422, detail=f"unknown seed: {seed!r}"
            )
    elif isinstance(seed, dict):
        if set(seed.keys()) != {"fork"} or not isinstance(seed.get("fork"), str):
            raise HTTPException(
                status_code=422, detail="seed must be {fork: <set_id>}"
            )
    else:
        raise HTTPException(status_code=422, detail="seed must be string or {fork}")
    try:
        meta = character_archive.create_set(
            character_id, name=body.name, seed=seed
        )
    except FileNotFoundError as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
    except character_archive.SetNotFound as exc:
        raise HTTPException(
            status_code=404, detail=f"set not found: {exc.args[0]}"
        ) from exc
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
...
async def flist_set_rename(
    character_id: str, set_id: str, body: SetRenameRequest
) -> dict:
    try:
        meta = character_archive.rename_set(
            character_id, set_id, name=body.name
        )
    except character_archive.SetNotFound:
...
async def flist_set_delete(
    character_id: str,
    set_id: str,
    next_active: str | None = None,
) -> dict:
    existing = character_archive.list_sets(character_id)
    ids = {s.id for s in existing}
    if set_id not in ids:
        raise HTTPException(status_code=404, detail="set not found")
    if len(existing) <= 1:
        raise HTTPException(status_code=409, detail="cannot delete only set")
    current_active = character_archive.read_active_set_id(character_id)
    if set_id == current_active:
        if not next_active:
            raise HTTPException(
                status_code=409,
                detail="must specify next_active when deleting active set",
            )
        if next_active == set_id or next_active not in ids:
            raise HTTPException(
                status_code=422,
                detail="next_active must be another existing set",
            )
        try:
            character_archive.set_active_set_id(character_id, next_active)
        except character_archive.SetNotFound:
            raise HTTPException(
                status_code=422, detail="next_active set not found"
            )
    deleted = character_archive.delete_set(character_id, set_id)
    return {
        "deleted": deleted,
        "new_active_set_id": character_archive.read_active_set_id(character_id),
    }


@app.post("/flist/character/{character_id}/sets/{set_id}/activate")
async def flist_set_activate(character_id: str, set_id: str) -> dict:
    try:
...
async def flist_set_payload_get(character_id: str, set_id: str) -> dict:
    try:
        payload = character_archive.read_set_payload(character_id, set_id)
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
    if payload is None:
        raise HTTPException(status_code=404, detail="set payload not found")
    etag = character_archive.set_payload_etag(character_id, set_id)
    return {"payload": payload, "etag": etag}


@app.put("/flist/character/{character_id}/sets/{set_id}/payload")
async def flist_set_payload_put(
    character_id: str,
    set_id: str,
    request: Request,
    if_match: str | None = Header(default=None, alias="If-Match"),
) -> dict:
    if character_archive.read_set_meta(character_id, set_id) is None:
        raise HTTPException(status_code=404, detail="set not found")
    try:
        body = await request.json()
    except ValueError as exc:
...
@app.get(
    "/flist/character/{character_id}/sets/{set_id}/snapshots"
)
async def flist_snapshot_list(character_id: str, set_id: str) -> dict:
    snaps = character_archive.list_snapshots(character_id, set_id)
    return {"snapshots": [_snapshot_meta_to_json(s) for s in snaps]}
...
async def flist_snapshot_create(
    character_id: str, set_id: str, body: SnapshotCreateRequest
) -> dict:
    try:
        snap = character_archive.write_snapshot(
            character_id, set_id, name=body.name
        )
    except character_archive.SetNotFound:
        raise HTTPException(status_code=404, detail="set not found")
        ...
        raise HTTPException(status_code=422, detail=str(exc)) from exc
    return {"snapshot": _snapshot_meta_to_json(snap)}


@app.patch(
    "/flist/character/{character_id}/sets/{set_id}/snapshots/{snap_id}"
)
async def flist_snapshot_rename(
    character_id: str,
    set_id: str,
    snap_id: str,
    ...
    try:
        snap = character_archive.rename_snapshot(
            character_id, set_id, snap_id, name=body.name
        )
    except character_archive.SnapshotNotFound:
...
async def flist_snapshot_delete(
    character_id: str, set_id: str, snap_id: str
) -> dict:
    try:
        deleted = character_archive.delete_snapshot(
            character_id, set_id, snap_id
        )
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
...
async def flist_snapshot_get(
    character_id: str, set_id: str, snap_id: str
) -> dict:
    try:
        snap = character_archive.find_snapshot_meta(
            character_id, set_id, snap_id
        )
        payload = character_archive.read_snapshot_payload(
            character_id, set_id, snap_id
        )
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
    if snap is None or payload is None:
        raise HTTPException(status_code=404, detail="snapshot not found")
    return {
        "snapshot": _snapshot_meta_to_json(snap),
        "payload": payload,
    }


@app.post(
    "/flist/character/{character_id}/sets/{set_id}/snapshots/{snap_id}/revert"
)
async def flist_snapshot_revert(
    character_id: str, set_id: str, snap_id: str
) -> dict:
    """Revert the set's payload to a snapshot. Auto-takes a safety
    snapshot of the current state first so the renderer can wire an
    explicit Undo on the 5-second banner."""
    try:
        target_payload = character_archive.read_snapshot_payload(
            character_id, set_id, snap_id
        )
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
    ...
    safety_name = (
        "Auto-safety @ "
        + character_archive._iso_timestamp_for_filename()
    )
    safety = character_archive.write_snapshot(
        character_id, set_id, name=safety_name
    )
    character_archive.write_set_payload(
        character_id, set_id, target_payload
    )
    meta = character_archive.read_set_meta(character_id, set_id)
    return {
        "set": _set_meta_to_json(meta) if meta is not None else None,
        "safety_snapshot_id": safety.id,
    }


@app.get("/flist/character/{character_id}/backups-v6")
async def flist_backups_v6_list(character_id: str) -> dict:
    rows = character_archive.list_backups_v6(character_id)
    return {"backups": [_backup_listing_to_json(r) for r in rows]}


@app.post("/flist/character/{character_id}/backups-v6", status_code=201)
async def flist_backups_v6_create(
    character_id: str, body: BackupCreateRequest
) -> dict:
    if body.source == "snapshot" and not body.snapshot_id:
        raise HTTPException(
            status_code=422, detail="snapshot_id required for snapshot source"
        )
    try:
        if body.source == "set":
            payload = character_archive.read_set_payload(
                character_id, body.set_id
            )
            source_label = "manual-set"
        else:
            payload = character_archive.read_snapshot_payload(
                character_id, body.set_id, body.snapshot_id  # type: ignore[arg-type]
            )
            source_label = "manual-snapshot"
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
    if payload is None:
        raise HTTPException(
            status_code=404,
            detail=f"{body.source} payload not found",
        )
    meta = character_archive.read_set_meta(character_id, body.set_id)
    if meta is None:
        raise HTTPException(status_code=404, detail="set not found")
    if body.source == "snapshot":
        snap_meta = character_archive.find_snapshot_meta(
            character_id, body.set_id, body.snapshot_id  # type: ignore[arg-type]
        )
        if snap_meta is None:
            raise HTTPException(status_code=404, detail="snapshot not found")
        source_name = snap_meta.name
    else:
        source_name = meta.name
    name_for_avatar: str | None = None
    char = payload.get("character") if isinstance(payload, dict) else None
    if isinstance(char, dict):
        nm = char.get("name")
        if isinstance(nm, str) and nm:
            name_for_avatar = nm
    avatar_path = (
        character_archive.avatar_path_for(name_for_avatar)
        if name_for_avatar
        else None
    )
    listing = character_archive.create_backup_from_payload(
        character_id,
        payload=payload,
        source=source_label,
    ...
    return {"backup": _backup_listing_to_json(listing)}


@app.delete("/flist/character/{character_id}/backups-v6/{filename}")
async def flist_backups_v6_delete(character_id: str, filename: str) -> dict:
    if not character_archive.BACKUP_FILENAME_RE.match(filename):
        raise HTTPException(status_code=400, detail="invalid backup filename")
    deleted = character_archive.delete_backup_v6(character_id, filename)
    if not deleted:
        raise HTTPException(status_code=404, detail="backup not found")
    return {"deleted": True}


@app.get("/flist/character/{character_id}/backups-v6/{filename}/path")
async def flist_backups_v6_path(character_id: str, filename: str) -> dict:
    if not (
        character_archive.BACKUP_FILENAME_RE.match(filename)
        or character_archive._BACKUP_FILE_RE.match(filename)
    ):
        raise HTTPException(status_code=400, detail="invalid backup filename")
    path = character_archive.backup_v6_path(character_id, filename)
    if path is None:
        raise HTTPException(status_code=404, detail="backup not found")
    return {"abs_path": str(path)}


@app.get("/flist/character/{character_id}/backups-v6/{filename}/download")
async def flist_backups_v6_download(
