...
def fake_data(tmp_path: Path) -> Path:
    ...
    char_a = tmp_path / "Aurora Fenwick" / "logs"
    char_a.mkdir(parents=True)
    (char_a / "Daelan Envale").write_bytes(b"\x00" * 1024)
    (char_a / "Daelan Envale.idx").write_bytes(b"\x00" * 16)
...
def test_list_characters(fake_data: Path) -> None:
    ...
    assert [c.name for c in chars] == ["Aurora Fenwick", "Vanessa Arlington"]
...
def test_list_partners_filters_idx_and_scratch(fake_data: Path) -> None:
    partners = list_partners("Aurora Fenwick", root=fake_data)
    names = [p.name for p in partners]
    assert names == ["#german ooc", "Daelan Envale"]
    # Sizes are real file sizes.
    sizes = {p.name: p.bytes for p in partners}
    assert sizes["Daelan Envale"] == 1024
...
def test_search_all_partners_empty_query(fake_data: Path) -> None:
    result = search_all_partners("Aurora Fenwick", "", root=fake_data)
    assert result == {"character": "Aurora Fenwick", "query": "", "hits": []}
...
def test_search_all_partners_returns_per_partner_shape(fake_data: Path) -> None:
    ...
    result = search_all_partners("Aurora Fenwick", "anything", root=fake_data)
    assert result["character"] == "Aurora Fenwick"
...
def test_find_contacts_matches_dm_partner_case_insensitive(fake_data: Path) -> None:
    # Aurora Fenwick has a "Daelan Envale" DM partner; query should
    # match regardless of case and exclude the named character itself
    # from results.
    res = find_contacts("DAELAN ENVALE", root=fake_data)
    assert res["name"] == "DAELAN ENVALE"
    chars = [d["character"] for d in res["dm"]]
    assert "Aurora Fenwick" in chars
    # Each DM entry carries partner + bytes for the renderer.
    aurora = next(d for d in res["dm"] if d["character"] == "Aurora Fenwick")
    assert aurora["partner"] == "Daelan Envale"
