...
def test_speaker_aliases_rewrite_in_text_and_speakers_field() -> None:
    ...
    long = "x" * 300
    old_name_msg = _mkmsg(ts(2026, 1, 1, 10), "Daelan Envale", long)
    new_name_msg = _mkmsg(ts(2026, 1, 1, 11), "Asmira", long)
    by_hash = {
    ...
    chunks = chunker.chunk_messages(
        ...
        labels_by_hash=by_hash,
        label_settings=_settings(),
        speaker_aliases=["Asmira", "Daelan Envale"],
    )
    assert len(chunks) == 1
    # Both speaker mentions in the chunk text collapsed to the primary.
    assert "Asmira:" in chunks[0]["text"]
    assert "Daelan Envale:" not in chunks[0]["text"]
    # Same for the speakers payload — deduped down to one entry.
    assert chunks[0]["speakers"] == ["Asmira"]
...
def test_speaker_aliases_leave_non_aliased_speakers_alone() -> None:
    ...
    long = "x" * 300
    old_msg = _mkmsg(ts(2026, 1, 1, 10), "Daelan Envale", long)
    ...
    assert "Daelan Envale:" not in chunks[0]["text"]
    assert chunks[0]["speakers"] == ["Asmira", "MyChar"]
...
def test_speaker_aliases_default_off_preserves_literal_speaker() -> None:
    ...
    long = "x" * 300
    m = _mkmsg(ts(2026, 1, 1, 10), "Daelan Envale", long)
    chunks = chunker.chunk_messages(
        [m],
        character="MyChar",
        partner="Asmira",
    ...
    assert "Daelan Envale:" in chunks[0]["text"]
    assert chunks[0]["speakers"] == ["Daelan Envale"]
