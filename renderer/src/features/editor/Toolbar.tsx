    ...
    <span className="tool-popover-wrap" ref={wrapRef}>
      ...
      {action.popover === 'eicon' && popoverOpen && (
        <EiconPopover
          onPick={(name) => {
            const view = viewRef.current
            if (view) applyAction(view, { label: 'eicon', title: 'Emote icon', insert: `[eicon]${name}[/eicon]` })
            setPopoverOpen(false)
          }}
          onClose={() => setPopoverOpen(false)}
        />
...
function EiconPopover({
  ...
  useEffect(() => {
    ...
    // Debounce so each keystroke doesn't hit the sidecar — Xariah
    // delta-fetched catalogs are big but the search is local, so 120 ms
    // is plenty.
    const t = setTimeout(() => {
      api
