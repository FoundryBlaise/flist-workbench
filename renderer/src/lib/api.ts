...
// ---- Phase 7 Tier 7 — working sets / snapshots / backup ZIPs ----
// Wire shapes (snake_case). The renderer state slice translates to
// camelCase via flist.ts helpers — kept separate so this file stays a
// pure HTTP boundary.
...
export type BackupListingWire = {
  filename: string
  createdAt: number
  size: number
  source: 'auto-pull' | 'manual-set' | 'manual-snapshot' | 'legacy-json'
...
export const api = {
  ...
  // ---- F-list working sets (Phase 7 Tier 7) ----
  flistSetsList: (characterId: string | number) =>
    get<{ sets: SetMetaWire[]; active_set_id: string }>(
      `/flist/character/${encodeURIComponent(String(characterId))}/sets`
    ),
  flistSetsCreate: (
    characterId: string | number,
    body: { name: string; seed: 'live' | 'empty' | { fork: string } }
  ) =>
  ...
  flistSetsDelete: (
    ...
    const qs = nextActiveSetId
      ? `?next_active=${encodeURIComponent(nextActiveSetId)}`
      : ''
    return request<{ deleted: boolean; new_active_set_id: string }>(
      `/flist/character/${encodeURIComponent(String(characterId))}/sets/${encodeURIComponent(setId)}${qs}`,
      { method: 'DELETE' }
    )
  },
  flistSetsActivate: (characterId: string | number, setId: string) =>
    request<{ active_set_id: string }>(
  ...
  flistSnapshotsList: (characterId: string | number, setId: string) =>
    get<{ snapshots: SnapshotMetaWire[] }>(
      `/flist/character/${encodeURIComponent(String(characterId))}/sets/${encodeURIComponent(setId)}/snapshots`
    ),
  flistSnapshotCreate: (
    characterId: string | number,
    setId: string,
    ...
    request<{ snapshot: SnapshotMetaWire }>(
      `/flist/character/${encodeURIComponent(String(characterId))}/sets/${encodeURIComponent(setId)}/snapshots`,
  ...
  flistSnapshotRename: (
    ...
    request<{ snapshot: SnapshotMetaWire }>(
      `/flist/character/${encodeURIComponent(String(characterId))}/sets/${encodeURIComponent(setId)}/snapshots/${encodeURIComponent(snapshotId)}`,
      { method: 'PATCH', body: JSON.stringify({ name }) }
    ),
  flistSnapshotDelete: (
    characterId: string | number,
  ...
  flistSnapshotRevert: (
    ...
    request<{ set: SetMetaWire; safety_snapshot_id: string }>(
      `/flist/character/${encodeURIComponent(String(characterId))}/sets/${encodeURIComponent(setId)}/snapshots/${encodeURIComponent(snapshotId)}/revert`,
      { method: 'POST' }
    ),
  flistSnapshotRead: (
    characterId: string | number,
    ...
    get<{ snapshot: SnapshotMetaWire; payload: Record<string, unknown> }>(
      `/flist/character/${encodeURIComponent(String(characterId))}/sets/${encodeURIComponent(setId)}/snapshots/${encodeURIComponent(snapshotId)}`
    ),
  // ---- F-list backups (Phase 7 Tier 7 — ZIP-shaped) ----
  // Note: sidecar exposes Tier 7 ZIPs under `/backups-v6/*` to avoid
  // colliding with the legacy `/backups[/...]` routes still consumed by
  // older flows. Step 16 collapses these once back-compat is dropped.
  flistBackupsListV6: (characterId: string | number) =>
    get<{ backups: BackupListingWire[] }>(
      `/flist/character/${encodeURIComponent(String(characterId))}/backups-v6`
    ),
  flistBackupCreate: (
    characterId: string | number,
    body:
      | { source: 'set'; set_id: string }
      | { source: 'snapshot'; set_id: string; snapshot_id: string }
  ) =>
    request<{ backup: BackupListingWire }>(
      `/flist/character/${encodeURIComponent(String(characterId))}/backups-v6`,
      { method: 'POST', body: JSON.stringify(body) }
    ),
  flistBackupDelete: (characterId: string | number, filename: string) =>
    request<{ deleted: boolean }>(
      `/flist/character/${encodeURIComponent(String(characterId))}/backups-v6/${encodeURIComponent(filename)}`,
      { method: 'DELETE' }
    ),
  flistBackupPath: (characterId: string | number, filename: string) =>
    get<{ abs_path: string }>(
      `/flist/character/${encodeURIComponent(String(characterId))}/backups-v6/${encodeURIComponent(filename)}/path`
    ),
